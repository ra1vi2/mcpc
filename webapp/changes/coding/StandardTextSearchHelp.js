sap.ui.define([
	...
	ImportStnText.prototype.open = function (oImportStnTextBtn) {
		...
		if (!this.oStandardTextSH) {
			return Fragment.load({
				...
				if (this._oView.getId() === sHeaderViewId || this._oView.getId() === sHierarchyHeaderViewId) {
	...
	ImportStnText.prototype.onFavoritePress = function (oEvent) {
		...
		var bIsFavorite = oStdTextObject.Favorite;
		var sFavOption = "";
		if (bIsFavorite) {
	 ...
	 * @param {object} oData - success result
	 * @param {object} oResponse - success response
	 * @return {object} will open standard text preview popover
	 */
	ImportStnText.prototype.handlePreviewSuccess = function (oData, oResponse) {
		...
		if (!this.oStandardTextPreviewPopover) {
			return Fragment.load({
				...
				this.oStandardTextResponsivePopover = Fragment.byId("idStandardTextPreviewFragment", "idStandardTextPreviewPopover");
				this.oStandardTextResponsivePopover.setModel(oPreviewModel, "previewModel");
				this._oView.addDependent(this.oStandardTextPreviewPopover);
				this.oStandardTextPreviewPopover.openBy(this.oPreviewIcon);
			}.bind(this));
		} else {
			this.oStandardTextResponsivePopover.getModel("previewModel").setData(oData);
			this.oStandardTextPreviewPopover.openBy(this.oPreviewIcon);
